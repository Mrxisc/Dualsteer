...
DS_MODEL="${DEEPSEEK_MODEL_PATH}"
XGUARD_MODEL="${XGUARD_MODEL_PATH:-${PROJECT_ROOT}/Models/YuFeng-XGuard-Reason-8B}"
RESULT_SUBDIR="DeepSeek-R1-Distill-Llama-8B_kg_filter_v2"
if [[ ! -f "$KG_NODES" ]]; then
  echo "[run_deepseek_kg_filter.sh] KG nodes not found. Building KG first..."
  bash "$KG_DIR/run_build_jailbreak_safety_kg.sh"
fi
echo "[run_deepseek_kg_filter.sh] DeepSeek-R1-Distill-Llama-8B with KG input filtering — $(date '+%Y-%m-%d %H:%M:%S')"
python "$KG_DIR/eval_qwen3_kg_filter.py" \
  ...
  --max-matches 20
echo "[run_deepseek_kg_filter.sh] XGuard ASR for KG-filter results — $(date '+%Y-%m-%d %H:%M:%S')"
...
echo "[run_deepseek_kg_filter.sh] Done — $(date '+%Y-%m-%d %H:%M:%S')"
