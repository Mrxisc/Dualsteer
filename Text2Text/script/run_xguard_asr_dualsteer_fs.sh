...
RESULT_DIR="$DAJA_ROOT/result/Qwen3-8B_dualsteer_fs"
source ${CONDA_ROOT}/etc/profile.d/conda.sh
conda activate PRISM
echo "[run_xguard_asr_dualsteer_fs.sh] Starting — $(date '+%Y-%m-%d %H:%M:%S')"
python "$TEST_SCRIPT" \
    ...
    --threshold 0.5 \
    --cuda 1
echo "[run_xguard_asr_dualsteer_fs.sh] Done — $(date '+%Y-%m-%d %H:%M:%S')"
