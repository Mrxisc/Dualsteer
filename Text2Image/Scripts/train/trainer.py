...
from Dualsteer_Code/Text2Image.Scripts.train.utils import geometric_median
...
class SaeTrainer:
    def __init__(self, cfg: TrainConfig, dataset_dict: dict[str, Dataset]):
        ...
        for hook, dataset in self.dataset_dict.items():
            feats = dataset.features["activations"].shape
            input_widths[hook] = feats[-1]
            if hook == list(dataset_dict.keys())[0]:
                self.sample_size = feats[-2]
        ...
        if getattr(self.cfg, "train_decoder_only", False):
            for sae in self.saes.values():
                sae.encoder.weight.requires_grad_(False)
                sae.encoder.bias.requires_grad_(False)
                if sae.W_dec is None:
                    raise RuntimeError(
                        "train_decoder_only=True but SAE decoder weights were not initialized (W_dec is None)."
                    )
                sae.W_dec.requires_grad_(True)
                sae.b_dec.requires_grad_(True)
        ...
        def _trainable_params(sae: Sae):
            if getattr(self.cfg, "train_decoder_only", False):
                assert sae.W_dec is not None
                return [sae.W_dec, sae.b_dec]
            return list(sae.parameters())


        pgs = []
        for sae in self.saes.values():
            params = [p for p in _trainable_params(sae) if p.requires_grad]
            if not params:
                raise RuntimeError("No trainable SAE parameters found. Check train_decoder_only configuration.")
            pgs.append(
                {
                    "params": params,
    ...
    def fit(self):
        ...
        for hook, ds in self.dataset_dict.items():
            ...
            if self.cfg.num_workers and self.cfg.num_workers > 0:
                dl_kwargs["persistent_workers"] = self.cfg.persistent_workers
                dl_kwargs["prefetch_factor"] = self.cfg.prefetch_factor
            dataloaders[hook] = DataLoader(ds, **dl_kwargs)
        pbar = tqdm(
            desc="Training",
    ...
    def save(self):
        base_path = self.cfg.save_dir or f"sae-ckpts/{self.cfg.wandb_project}"
        path = f"{base_path}/{self.cfg.run_name}" if self.cfg.run_name else base_path
        rank_zero = not dist.is_initialized() or dist.get_rank() == 0
        ...
        if rank_zero:
            ...
            try:
                ...
                with open(cfg_path, "w", encoding="utf-8") as f:
                    json.dump(asdict(self.cfg), f, indent=2, default=_json_default)
