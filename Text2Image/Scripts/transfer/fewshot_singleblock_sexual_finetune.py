...
DEFAULT_HOOKPOINT = "transformer.single_transformer_blocks.37"
DEFAULT_BASE_SAE_DIR = "SAEs/flux_real_singleblock37_i2p_no_sexual_i2p_no_sexual_flux_real"
DEFAULT_ACTIVATIONS_DIR = "Activations/i2p_sexual_flux"
DEFAULT_CANONICAL_ACTIVATION_DIRNAME = "flux_model.singleblock37"
DEFAULT_OUTPUT_SAE_DIR = "SAEs/flux_singleblock37_sexual_transfer"
DEFAULT_REPLAY_ACTIVATIONS_DIR = "Activations/i2p_no_sexual_flux_real"
DEFAULT_FLUX_CONFIG = "_Backup_code/configs/flux1_dev_real.json"  # kept for CLI compatibility
DEFAULT_CACHE_EVERY_N = 1
DEFAULT_TOKEN_SUBSAMPLE = 256
DEFAULT_CHANNEL_PROJ_DIM = 1024
DEFAULT_TARGET_ACTIVATION_ROWS = 900
DEFAULT_TARGET_ACTIVATION_SHARDS = 900
...
def _count_hf_shards(hook_dir: Path) -> int:
	...
	if state_path.exists():
		try:
			...
			pass
	return len(list(hook_dir.glob("data-*.arrow")))
	...
	*,
	...
	try:
		...
		if channel_proj_dim is not None and int(meta.get("channel_proj_dim", -1)) != int(channel_proj_dim):
			return False
		if int(meta.get("target_activation_rows", -1)) != int(required_rows):
			return False
		if int(meta.get("target_activation_shards", -1)) != int(required_shards):
			return False
	except Exception:
		return False
	...
	try:
		...
		ds = load_from_disk(str(hook_dir))
		if len(ds) != int(required_rows):
			return False
		if _count_hf_shards(hook_dir) != int(required_shards):
			return False
	except Exception:
		return False
...
def _collect_real_activations_if_needed(args: argparse.Namespace) -> None:
	cache_ok = _hook_cache_ready(
		...
		required_rows=args.target_activation_rows,
		required_shards=args.target_activation_shards,
	)
	if args.force_cache or not cache_ok:
		...
		# Match source d_in using the replay channel projection.
		...
		try:
			...
			shard_count = _count_hf_shards(canonical_dir)
			if n != int(args.target_activation_rows) or shard_count != int(args.target_activation_shards):
				print(
					f"[cache-real] Rewriting dataset for exact rows/shards: rows={len(ds)} shards={shard_count} "
					f"-> rows={args.target_activation_rows} shards={args.target_activation_shards}"
				)
				tmp = canonical_dir.parent / (canonical_dir.name + ".tmp_reshard")
				if tmp.exists():
					shutil.rmtree(tmp)
				ds.save_to_disk(str(tmp), num_shards=int(args.target_activation_shards))
				shutil.rmtree(canonical_dir)
				tmp.replace(canonical_dir)

		except Exception as e:
			raise RuntimeError(f"Failed to validate/enforce activation cache at {canonical_dir}: {e}")
		...
		meta = {
			...
			"target_activation_rows": int(args.target_activation_rows),
			"target_activation_shards": int(args.target_activation_shards),
		}
...
def parse_args() -> argparse.Namespace:
	p = argparse.ArgumentParser(description="Few-shot finetune Flux singleblock37 SAE on i2p sexual subset")

	p.add_argument("--base-sae-dir", default=DEFAULT_BASE_SAE_DIR)
	p.add_argument("--hookpoint", default=DEFAULT_HOOKPOINT, help="Singleblock hookpoint to finetune")
	...
	p.add_argument(
		...
		help="Require exactly N activation rows in activations/<flux_model.singleblock37>.",
	)
	p.add_argument(
		"--target-activation-shards",
		type=int,
		default=DEFAULT_TARGET_ACTIVATION_SHARDS,
		help="Require exactly N shards (data-*.arrow) in the saved dataset.",
	)
	...
	p.add_argument("--cache-every-n", type=int, default=DEFAULT_CACHE_EVERY_N)
	...
	p.add_argument(
		...
		help="Source-domain cached activations for replay (default: match transfer recipe).",
	)
	...
	if args.log_file is None:
		...
		args.log_file = str(log_dir / f"transfer_flux_singleblock37_{ts}.log")
	else:
...
def main() -> None:
	...
	print("=== Few-shot singleblock37 sexual finetune (hardcoded defaults) ===")
	...
	print(f"require: rows={args.target_activation_rows} shards={args.target_activation_shards}")
