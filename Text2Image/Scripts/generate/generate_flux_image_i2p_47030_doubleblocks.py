...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Generate I2P images with Flux + doubleblock SAE protection")
    ...
    parser.add_argument(
        "--save-root",
        default=str(ROOT_DIR / "Results" / "i2p_flux_doubleblock"),
    )
    parser.add_argument(
        "--sae-double-checkpoint",
        default=str(ROOT_DIR / "SAEs" / "flux_real_doubleblock18_i2p_no_sexual_i2p_no_sexual_flux_real"),
        help="Checkpoint root for the doubleblock SAE",
    )
    parser.add_argument(
        "--double-hookpoint",
    ...
    parser.add_argument(
        ...
        help="Activation cache root (used to load channel_proj.pt / token_indices.pt)",
    )
    parser.add_argument(
        "--sae-tag",
        default="doubleblock18",
        help="Tag name for output subdir and manifest (e.g., doubleblock18_s0.2)",
    )
...
def main():
    ...
    sae_path = _resolve_checkpoint_dir(args.sae_double_checkpoint, args.double_hookpoint)
    if sae_path is None:
        raise FileNotFoundError(f"Cannot find sae.safetensors under {args.sae_double_checkpoint} (hookpoint={args.double_hookpoint})")


    double_sae = Sae.load_from_disk(sae_path, device=device).eval()
    double_sae.cfg.batch_topk = False
    double_sae.cfg.sample_topk = False
    ...
    print(f"Running doubleblock SAE '{args.sae_tag}' -> output {out_root}")
    ...
    generate_for_sae(
        ...
        hook_fn=hook_fn,
        hookpoint=args.double_hookpoint,
        rows=rows,
        out_root=out_root,
        steps=args.steps,
