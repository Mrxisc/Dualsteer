...
class Detector:
    ...
    def __init__(self, model_name="base"):
        ...
        env_override = os.environ.get("NUDENET_DIR")
        ...
        default_home = os.path.join(home, ".NudeNet")


        if env_override and os.path.isdir(env_override):
            model_folder = env_override
        elif os.path.isdir(repo_local):
            model_folder = repo_local
        else:
            model_folder = default_home
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Quick NudeNet detector smoke test")
    parser.add_argument("--image", type=str, required=True, help="Path to a local image")
    args = parser.parse_args()


    m = Detector()
    img = PIL.Image.open(args.image)
    print(m.detect(args.image))
