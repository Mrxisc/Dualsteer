...
SAE_ROOT="${SAE_ROOT:-${ROOT_DIR}/SAEs/sd35_block36_i2p_no_sexual_activations}"
MMDIT_HOOKPOINT=${MMDIT_HOOKPOINT:-"transformer.transformer_blocks.36"}

ACTIVATIONS_ROOT=${ACTIVATIONS_ROOT:-"${ROOT_DIR}/Activations/i2p_no_sexual_SD_real"}
...
LOG_FILE="${LOG_DIR}/generate_sd35_image_i2p47030_mmdit_${TS}.log"

{
  ...
  echo "mmdit_hookpoint=${MMDIT_HOOKPOINT}"
  ...
  python "${ROOT_DIR}/Scripts/generate/generate_SD35_image_i2p_47030_mmdit.py" \
    --sd35-ckpt "${SD35_CKPT}" \
    --prompts-csv "${PROMPTS_CSV}" \
    --save-root "${SAVE_ROOT}" \
    --sae-root "${SAE_ROOT}" \
    --mmdit-hookpoint "${MMDIT_HOOKPOINT}" \
