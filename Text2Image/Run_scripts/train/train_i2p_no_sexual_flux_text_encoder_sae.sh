...
DATASET_ROOT=${DATASET_ROOT:-${TEXT2IMAGE_DIR}/Activations/i2p_no_sexual_flux}
HOOK_NAME=${HOOK_NAME:-flux_model.text_encoder}
RUN_NAME=${RUN_NAME:-flux_text_encoder_i2p_no_sexual}
WANDB_PROJECT=${WANDB_PROJECT:-dualsteer_flux_i2p}
...
LOG_FILE="${LOG_DIR}/train_${HOOK_NAME//./_}_${TIMESTAMP}.log"

ARGS=(
  "--dataset_path" "${DATASET_ROOT}"
  "--hookpoints" "${HOOK_NAME}"
  "--effective_batch_size" "${EFFECTIVE_BATCH_SIZE:-8192}"
  ...
  "--micro_acc_steps" "${MICRO_ACC_STEPS:-2}"
...
{
  echo "=== SAE Training (Flux text encoder) ==="
  echo "timestamp=${TIMESTAMP}"
  echo "hook=${HOOK_NAME}"
  echo "dataset_path=${DATASET_ROOT}"
  echo "run_name=${RUN_NAME}"
  echo "save_dir=${TEXT2IMAGE_DIR}/SAEs"
  echo "effective_batch_size=${EFFECTIVE_BATCH_SIZE:-8192}"
  echo "num_epochs=${NUM_EPOCHS:-3}"
  echo "lr=${LR:-3e-4}"
  ...
  echo "micro_acc_steps=${MICRO_ACC_STEPS:-2}"
