...
BASE_SAE_DIR=${BASE_SAE_DIR:-${TEXT2IMAGE_DIR}/SAEs/sd35_block36_i2p_no_sexual_activations}

TARGET_ACTIVATIONS_DIR=${TARGET_ACTIVATIONS_DIR:-${TEXT2IMAGE_DIR}/Activations/i2p_sexual_SD_real}
REPLAY_ACTIVATIONS_DIR=${REPLAY_ACTIVATIONS_DIR:-${TEXT2IMAGE_DIR}/Activations/i2p_no_sexual_SD_real}

OUTPUT_SAE_DIR=${OUTPUT_SAE_DIR:-${TEXT2IMAGE_DIR}/SAEs/sd35_block36_sexual_transfer}

LOG_DIR=${LOG_DIR:-${TEXT2IMAGE_DIR}/Logs}
mkdir -p "${LOG_DIR}"
TIMESTAMP=$(date +"%Y%m%d_%H%M%S")
LOG_FILE="${LOG_DIR}/transfer_sd35_block_real_${TIMESTAMP}.log"
...
{
	echo "=== SD3.5 REAL MMDiT Block SAE Transfer ==="
	...
	echo "token_subsample=${TOKEN_SUBSAMPLE:-<none>} channel_proj_dim=${CHANNEL_PROJ_DIM} channel_proj_path=${CHANNEL_PROJ_PATH}"
	...
	if [[ "${FORCE_CACHE}" == "1" ]] || ! hook_cache_ready "${TARGET_ACTIVATIONS_DIR}" "${HOOK_NAME}"; then
		...
		if [[ -n "${TOKEN_SUBSAMPLE}" ]]; then
			CACHE_ARGS+=("--token_subsample" "${TOKEN_SUBSAMPLE}")
		fi
