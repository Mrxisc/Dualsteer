...
OUTPUT_ROOT="${TEXT2IMAGE_ROOT}/Activations/i2p_no_sexual_SD_real"
...
TOKEN_SUBSAMPLE="${TOKEN_SUBSAMPLE:-512}"
...
MMDIT_HOOK="${MMDIT_HOOK:-transformer.transformer_blocks.36}"
...
LOG_FILE="${LOG_DIR}/collect_i2p_no_sexual_sd35_real_mmdit_${TIMESTAMP}.log"

python "${CACHE_SCRIPT}" \
  --backend sd35 \
  --model_name "${MODEL_DIR}" \
  --hook_names "${MMDIT_HOOK}" \
